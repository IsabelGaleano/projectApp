...
export class InscripcionesRoutingResolveService implements Resolve<IInscripciones> {
  constructor(protected service: InscripcionesService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IInscripciones> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((inscripciones: HttpResponse<Inscripciones>) => {

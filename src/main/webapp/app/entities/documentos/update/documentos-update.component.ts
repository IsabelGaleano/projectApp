...
export class DocumentosUpdateComponent implements OnInit {
  ...
  editForm = this.fb.group({
    ...
    url: [null, [Validators.minLength(1), Validators.maxLength(300)]],
  ...
  save(): void {
    this.isSaving = true;
    const documentos = this.createFromForm();
    if (documentos.id !== undefined) {
      this.subscribeToSaveResponse(this.documentosService.update(documentos));
    } else {
      this.subscribeToSaveResponse(this.documentosService.create(documentos));
    }
  }
  ...
  protected subscribeToSaveResponse(result: Observable<HttpResponse<IDocumentos>>): void {
  ...
  protected updateForm(documentos: IDocumentos): void {
  ...
  protected createFromForm(): IDocumentos {
    return {
      ...new Documentos(),
